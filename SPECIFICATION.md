...
published or required. The installer displays the detected LAN IPv4 address
and the hostname-to-address mapping that the administrator must configure in
local DNS or on client devices; it does not modify external DNS systems.
...
The installer prompts once for a valid dot-separated hostname and stores it in
`/opt/vaultwarden/.caddy-hostname`. The default is `vaultwarden.local`.
...
On reruns, the installer preserves the stored hostname, Caddy configuration,
persistent data and internal CA. It does not silently change the hostname or
regenerate the CA.
